...
type JobQuotaLister interface {
	...
	List(selector labels.Selector) (ret []*api.JobQuota, err error)
...
// List lists all JobQuotas in the indexer.
func (s *jobQuotaLister) List(selector labels.Selector) (ret []*api.JobQuota, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*api.JobQuota))
	})
	return ret, err
}
...
type JobQuotaNamespaceLister interface {
	...
	Get(name string) (*api.JobQuota, error)
...
// List lists all JobQuotas in the indexer for a given namespace.
func (s jobQuotaNamespaceLister) List(selector labels.Selector) (ret []*api.JobQuota, err error) {
...
// Get retrieves the JobQuota from the indexer for a given namespace and name.
func (s jobQuotaNamespaceLister) Get(name string) (*api.JobQuota, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(api.Resource("jobquota"), name)
	}
	return obj.(*api.JobQuota), nil
}
